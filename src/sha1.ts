export function sha1_hmac(msg: string, key: string): string {
    "use strict";
...
function sha1(msg: string, raw: boolean = false) {
    ...
    function cvt_hex(val: number, raw: boolean = false): string {
        var str = "";
        var i;
        var v;
    ...
    for (blockstart = 0; blockstart < word_array.length; blockstart += 16) {
        ...
        for (i = 0; i <= 19; i++) {
            var temp: number = (rotate_left(A, 5) + ((B & C) | (~B & D)) + E + W[i] + 0x5A827999) & 0x0ffffffff;
            E = D;
            D = C;
            C = rotate_left(B, 30);
            B = A;
